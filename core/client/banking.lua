...
AddEventHandler("playerSpawned", function()
    local account = lib.callback.await(BANKING.events.GET_INFO)

    if not account then return end


    StatSetInt("BANK_BALANCE", account.balance, false)
    SetMultiplayerBankCash();

    StatSetInt("MP0_WALLET_BALANCE", account.wallet, false)
